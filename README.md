...
在本仓库 Releases 下载 `dsh-media-workbench-0.4.0.tgz`，这是 DSH 插件包，不是 Desktop 安装程序。仓库根目录也是可安装的 DSH Bundle，可以直接作为插件源码目录使用。v0.4.0 支持直接下拉修改选题状态，选择已排期须确认日期时间；新增或调整排期自动变为已排期。选题详情可以添加发布链接或关联已有未归属发布记录，统一进入数据采集与监控。
...
```sh
gh release download v0.4.0 --repo cinderzhan/dsh-media-workbench --pattern '*.tgz'
mkdir dsh-media-workbench-install
tar -xzf dsh-media-workbench-0.4.0.tgz -C dsh-media-workbench-install --strip-components=1
```
