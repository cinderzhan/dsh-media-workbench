...
describe('dashboard configuration', () => {
  it('saves independent chart settings, additions and deletion across remounts', () => {
    ...
    target.querySelector('.analytics-card-heading button[aria-label^=删除]').click()
    target = mount()
...
it('keeps the editor explicit and preserves a title draft and cursor during data refresh', () => {
  const state = { workspaceId: 'title-refresh', publications: [publication('a')], snapshots: [snapshot('a', null, '2026-09-11T10:00:00Z')] }
  const target = document.createElement('section'); document.body.append(target); renderAnalytics(target, state)
  const edit = target.querySelector('button[aria-label^="编辑图表"]')
  expect(edit.closest('.analytics-card-heading')).not.toBeNull()
  expect(edit.getAttribute('aria-expanded')).toBe('false')
  edit.click()
  const input = target.querySelector('input[id$="title"]'); input.focus(); input.value = '  本周趋势'; input.dispatchEvent(new Event('input')); input.setSelectionRange(3, 3)
  renderAnalytics(target, { ...state, snapshots: [...state.snapshots] })
  expect(target.querySelector('.analytics-editor').open).toBe(true)
  expect(document.activeElement.value).toBe('  本周趋势')
  expect(document.activeElement.selectionStart).toBe(3)
  expect(target.querySelector('h3').textContent).toBe('本周趋势')
  const details = target.querySelector('.analytics-data-details')
  expect(details.open).toBe(false)
  expect(details.textContent).toContain('缺失值显示为 —')
  expect(details.querySelector('table')).not.toBeNull()
  target.querySelector('button[aria-label^="编辑图表"]').click()
